...
export function FilterSidebar({ filters, setFilters, isMobile }: FilterSidebarProps) {
  ...
  return (
    <aside className={isMobile ? "w-full" : "w-full md:w-64 shrink-0 md:border-r border-white/10 md:pr-8 md:sticky md:top-24 self-start h-auto md:h-[calc(100vh-6rem)] overflow-y-auto hidden md:block custom-scrollbar pb-10"}>
      <div className="space-y-8 bg-[#0a0a0b] text-zinc-100">
        <div>
          <h3 className="text-xl font-black text-white mb-6 uppercase tracking-widest text-sm opacity-80 border-b border-white/10 pb-3">Filters</h3>
        </div>

        {/* Status Filter */}
        <div className="bg-white/[0.02] p-4 rounded-2xl border border-white/5">
          <h4 className="font-bold text-zinc-300 text-xs mb-4 uppercase tracking-wider flex items-center gap-2">
            ...
            {["all", "live", "scheduled"].map((status) => (
              <label key={status} className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 text-indigo-500 bg-white/5 border-white/10 focus:ring-indigo-500/50 focus:ring-offset-[#0a0a0b]"
                />
                <span className="text-sm font-medium text-zinc-400 group-hover:text-zinc-100 capitalize transition-colors">{status}</span>
              </label>
            ))}
          </div>
        </div>
            ...
            {eventTypes.map((type) => (
              <label key={type} className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 rounded bg-white/5 border-white/10 text-indigo-500 focus:ring-indigo-500/50 focus:ring-offset-[#0a0a0b]"
                />
                <span className="text-sm font-medium text-zinc-400 group-hover:text-zinc-100 capitalize transition-colors">{type}</span>
              </label>
            ))}
          </div>
        </div>
            ...
            {["all", "free", "paid"].map((cost) => (
              <label key={cost} className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 text-indigo-500 bg-white/5 border-white/10 focus:ring-indigo-500/50 focus:ring-offset-[#0a0a0b]"
                />
                <span className="text-sm font-medium text-zinc-400 group-hover:text-zinc-100 capitalize transition-colors">{cost}</span>
              </label>
            ))}
          </div>
        </div>
